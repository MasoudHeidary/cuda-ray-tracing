...
__host__ __device__ glm::vec3 compute_lighting(const glm::vec3& point, const glm::vec3& normal, Light* lights, int num_light) {
    ...
    for(int _li=0; _li<num_light; _li++) {
        glm::vec3 light_dir = glm::normalize(lights[_li].position - point);
        float diffuse_intensity = glm::max(glm::dot(normal, light_dir), 0.0f);
        light_color += (diffuse_intensity * lights[_li].color);
        // printf("lightning color: %f %f %f\n", lighting.r, lighting.g, lighting.b);
    }

    // printf("compute_lightning light_color: %f %f %f\n", light_color.r, light_color.g, light_color.b);
